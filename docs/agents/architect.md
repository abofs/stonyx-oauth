...
- **State token validation (#36):** CSRF protection via `StateStore` — a `crypto.randomUUID()` state bound to a 32-byte client-held binding value. `StateStore.pending` maps the state to `{ provider, bindingHash, createdAt }`; the plaintext binding value is held only by the client, as an HttpOnly cookie. A callback is accepted only when *one of* the presented binding values hashes to `bindingHash`, for the same provider, within `STATE_TTL_MS` — all of them are tried, because a client can hold more than one cookie of that name and reading only the first is a denial-of-login vector. States are single-use and are consumed on any callback that presents a recognised state, successful or not; the binding cookie is cleared only on a request that consumed one
