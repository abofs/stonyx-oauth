...
- **State token tests (#36):** Verify that `getAuthorizationUrl()` returns `{ url, bindingValue }` and records `{ provider, bindingHash, createdAt }` in `StateStore.pending`, and that `handleCallback(provider, code, state, bindingValues)` — whose fourth argument is required, and is the array of *every* value the client presented under the binding cookie's name — rejects a callback presenting no binding value, another client's binding value, the stored `bindingHash` itself, a state issued for a different provider, an expired state, or an unknown state, and *accepts* one where a planted same-named cookie is presented alongside the real value. Drive it through the real `OAuth` instance; a test that re-implements the check measures nothing (see `test/unit/state-validation-test.ts`)
