...
- **State token tests (#36):** Verify that `getAuthorizationUrl()` returns `{ url, bindingValue }` and records `{ provider, bindingHash, createdAt }` in `StateStore.pending`, and that `handleCallback(provider, code, state, bindingValue)` — whose fourth argument is required — rejects a callback presenting no binding value, another client's binding value, the stored `bindingHash` itself, a state issued for a different provider, an expired state, or an unknown state. Drive it through the real `OAuth` instance; a test that re-implements the check measures nothing (see `test/unit/state-validation-test.ts`)
